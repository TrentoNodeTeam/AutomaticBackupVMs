...
list_vms() {
   nova list --all-tenants --host $node | grep -v "(+------|\| ID)" | \
   igawk -F '|' '
   @include trims.awk
...
rsync -avz -e "ssh -i /root/key_cmpt" /var/lib/glance/image4migration/snapshot-$vm.qcow root@10.20.0.4:/var/lib/glance/imagesBackup/
